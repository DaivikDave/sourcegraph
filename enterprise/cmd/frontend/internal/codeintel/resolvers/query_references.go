...
// uploadIDsWithReferences returns uploads that probably contain an import or implementation moniker
// whose identifier matches any of the given monikers' identifiers. This method will not return uploads
// for commits which are unknown to gitserver, nor will it return uploads which are listed in the given
// ignored identifier slice. This method also returns the number of records scanned (but possibly
// filtered out from the return slice) from the database (the offset for the subsequent request) and the
